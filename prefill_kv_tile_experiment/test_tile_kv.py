...
from bench_utils import get_prefill_tile_params, bench_ms, attention_flops, tflops
...
# ── 정확도 검증 (FlashAttention과 즉석 비교) ─────────────────

def _reconstruct_dense_kv(kv_cache, kv_indptr, kv_indices, kv_last, seq_lens, page_size):
    """paged KV cache → dense (total_tokens, num_kv_heads, head_dim)"""
    total_tokens = sum(seq_lens)
    num_kv_heads = kv_cache.shape[3]
    head_dim     = kv_cache.shape[4]
    k = torch.empty(total_tokens, num_kv_heads, head_dim, dtype=kv_cache.dtype, device=kv_cache.device)
    v = torch.empty(total_tokens, num_kv_heads, head_dim, dtype=kv_cache.dtype, device=kv_cache.device)
    ...
    for i, seq_len in enumerate(seq_lens):
        n_pages = indptr_cpu[i + 1].item() - indptr_cpu[i].item()
        for p in range(n_pages):
            pid    = indices_cpu[indptr_cpu[i].item() + p].item()
            n_tok  = page_size if p < n_pages - 1 else last_cpu[i].item()
            k[token_off:token_off + n_tok] = kv_cache[pid, 0, :n_tok]
            v[token_off:token_off + n_tok] = kv_cache[pid, 1, :n_tok]
            token_off += n_tok
    return k, v


def correctness_check(seq_len, num_qo_heads, num_kv_heads, head_dim, batch_size, page_size,
                      q, kv_cache, kv_indptr, kv_indices, kv_last, out_fi):
    """FlashAttention 출력과 비교 (디스크 저장 없음)"""
    seq_lens = [seq_len] * batch_size
    k, v = _reconstruct_dense_kv(kv_cache, kv_indptr, kv_indices, kv_last, seq_lens, page_size)

    cu_seqlens = torch.arange(
        0, (batch_size + 1) * seq_len, seq_len, dtype=torch.int32, device=DEVICE
    )
    out_fa = flash_attn_varlen_func(q, k, v, cu_seqlens, cu_seqlens,
                                    seq_len, seq_len, causal=True)

    max_err  = (out_fi - out_fa).abs().max().item()
    mean_err = (out_fi - out_fa).abs().mean().item()
...
def run(label, num_qo_heads, num_kv_heads, head_dim, batch_size, page_size, skip_correctness=False):
    print(SEP)
    print(f"  label={label}")
    ...
    for seq_len in SEQ_LENS:
        ...
        if skip_correctness:
            ...
            corr   = correctness_check(seq_len, num_qo_heads, num_kv_heads, head_dim,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skip_correctness", action="store_true",     help="correctness check 생략 (baseline 실험 시 권장)")
